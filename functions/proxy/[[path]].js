...
export async function onRequest(context) {
    ...
    async function processMasterPlaylist(url, content, recursionDepth, env) {
        ...
        try {
            kvNamespace = env.TanhaxTV_PROXY_KV; // 从环境获取 KV 命名空间 (变量名在 Cloudflare 设置)
            if (!kvNamespace) throw new Error("KV 命名空间未绑定");
        } catch (e) {
            logDebug(`KV 命名空间 'TanhaxTV_PROXY_KV' 访问出错或未绑定: ${e.message}`);
    ...
    try {
        ...
        let kvNamespace = null;
        try {
            kvNamespace = env.TanhaxTV_PROXY_KV;
            if (!kvNamespace) throw new Error("KV 命名空间未绑定");
        } catch (e) {
            logDebug(`KV 命名空间 'TanhaxTV_PROXY_KV' 访问出错或未绑定: ${e.message}`);
            kvNamespace = null;
