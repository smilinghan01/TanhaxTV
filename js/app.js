...
let allApiKeys = Object.keys(API_SITES);
...
function initAPICheckboxes() {
    ...
    Object.keys(API_SITES).forEach(apiKey => {
        ...
        checkbox.querySelector('input').addEventListener('change', function () {
            updateSelectedAPIs();
            // 黄色功能已注释 - checkAdultAPIsSelected();
        });
    });
...
function renderCustomAPIsList() {
    ...
    customAPIs.forEach((api, index) => {
        ...
        const textColorClass = 'text-white'; // 黄色功能已注释 - api.isAdult ? 'text-pink-400' : 'text-white';
        // 黄色功能已注释 - const adultTag = api.isAdult ? '<span class="text-xs text-pink-400 mr-1">(18+)</span>' : '';
        // 新增 detail 地址显示
...
function updateCustomApi(index) {
    ...
    // 黄色功能已注释 - const isAdult = isAdultInput ? isAdultInput.checked : false;
    if (!name || !url) {
    ...
    // 黄色功能已注释 - if (isAdultInput) isAdultInput.checked = false;
...
function cancelEditCustomApi() {
    ...
    // 黄色功能已注释 - const isAdultInput = document.getElementById('customApiIsAdult');
...
function toggleSettings(e) {
    ...
    if (settingsPanel.classList.contains('show')) {
        ...
        if (favoritesPanel) favoritesPanel.classList.remove('show');
    }
...
function setupEventListeners() {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (settingsPanel && !settingsPanel.contains(e.target) && !e.target.closest('button[onclick*="toggleSettings"]')) {
            settingsPanel.classList.remove('show');
        }

        // 关闭历史记录面板
        const historyPanel = document.querySelector('#historyPanel.show');
        if (historyPanel && !historyPanel.contains(e.target) && !e.target.closest('button[onclick*="toggleHistory"]')) {
            historyPanel.classList.remove('show');
        }

        // 关闭收藏面板
        const favoritesPanel = document.querySelector('#favoritesPanel.show');
        if (favoritesPanel && !favoritesPanel.contains(e.target) && !e.target.closest('button[onclick*="toggleFavoritesPanel"]')) {
            favoritesPanel.classList.remove('show');
        }
    });

    // 黄色功能已注释 - 黄色内容过滤开关事件绑定
    /*
    const yellowFilterToggle = document.getElementById('yellowFilterToggle');
    if (yellowFilterToggle) {
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `TanhaxTV - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `TanhaxTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - TanhaxTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
// 渲染搜索结果到页面
function renderSearchResults(allResults) {
    const resultsDiv = document.getElementById('results');

    // 黄色功能已注释 - 处理搜索结果过滤：如果启用了黄色内容过滤，则过滤掉分类含有敏感内容的项目
    /*
    ...
    */
    ...
    allResults = [...allResults].sort((a, b) => {
        switch (sortOrder) {
            case 'name-desc': return (b.vod_name || '').localeCompare(a.vod_name || '');
            case 'year-desc': return (parseInt(b.vod_year) || 0) - (parseInt(a.vod_year) || 0);
            case 'year-asc': return (parseInt(a.vod_year) || 0) - (parseInt(b.vod_year) || 0);
            case 'source-asc': return (a.source_name || '').localeCompare(b.source_name || '');
            case 'source-desc': return (b.source_name || '').localeCompare(a.source_name || '');
            default: return (a.vod_name || '').localeCompare(b.vod_name || '');
        }
    });
    ...
    if (filterKeyword) {
        allResults = allResults.filter(item => 
            (item.vod_name || '').toLowerCase().includes(filterKeyword)
        );
    }
                    ...
                    <div class="p-2 flex flex-col flex-grow">
                        <div class="flex-grow">
                            ...
                            <div class="flex flex-wrap ${hasCover ? '' : 'justify-center'} gap-1 mb-2">
                                ...
                                ${(item.vod_year || '') ?
                                `<span class="text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-purple-500 text-purple-300">${item.vod_year}</span>` : ''}
                            </div>
                        ...
                        <div class="flex justify-between items-center mt-1 pt-1 border-t border-gray-800">
                            ...
                            <button onclick="event.stopPropagation(); toggleFavorite({vod_id: '${safeId}', vod_name: '${safeName.replace(/'/g, "\\'")}', vod_pic: '${(item.vod_pic || '').replace(/'/g, "\\'")}', vod_year: '${item.vod_year || ''}', type_name: '${(item.type_name || '').replace(/</g, '&lt;').replace(/'/g, "\\'")}', vod_remarks: '${(item.vod_remarks || '').replace(/'/g, "\\'")}', source_code: '${sourceCode}', source_name: '${item.source_name || ''}', api_url: '${(item.api_url || '').replace(/'/g, "\\'")}'})"
                                    class="text-gray-500 hover:text-red-400 transition-colors p-1 ${isFavorited('${safeId}', '${sourceCode}') ? 'text-red-400' : ''}"
                                    title="${isFavorited('${safeId}', '${sourceCode}') ? '取消收藏' : '收藏'}">
                                <svg class="w-4 h-4" fill="${isFavorited('${safeId}', '${sourceCode}') ? 'currentColor' : 'none'}" stroke="currentColor" viewBox="0 0 24 24">
...
async function showDetails(id, vod_name, sourceCode) {
    ...
    try {
        ...
        // 不对标题进行截断处理，允许完整显示
        const favoriteIcon = isFavorited(id, sourceCode) ? 'text-red-400' : 'text-gray-400 hover:text-red-400';
        const favoriteFill = isFavorited(id, sourceCode) ? 'currentColor' : 'none';
        modalTitle.innerHTML = `<span class="break-words flex items-center gap-2">${vod_name || '未知视频'}${sourceName}
            <button onclick="toggleFavorite({vod_id: '${id.replace(/'/g, "\\'")}', vod_name: '${(vod_name || '').replace(/'/g, "\\'")}', vod_pic: '${(data.videoInfo?.cover || '').replace(/'/g, "\\'")}', source_code: '${sourceCode}', source_name: '${data.videoInfo?.source_name || ''}'})" 
                    class="${favoriteIcon} p-1 transition-colors" title="收藏">
                <svg class="w-5 h-5" fill="${favoriteFill}" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                </svg>
            </button>
        </span>`;
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'TanhaxTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'TanhaxTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'TanhaxTV-Settings_' + times + '.json');
}
